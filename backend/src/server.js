...
app.get("/health", async (_req,res,next) => {
  try { await pool.query("SELECT 1"); res.json({ status:"ok",service:"fodd-api",version:"7.0" }); }
  catch(e){ next(e); }
});
...
app.post("/api/together/plans",requireAuth,async(req,res,next)=>{
  ...
  try{
    ...
    for(const id of memberIds){if(await pairBlocked(req.user.id,id))return res.status(403).json({error:"Salah satu akun tidak dapat diundang"});}
...
app.listen(port,"0.0.0.0",()=>console.log(`Fodd API v6.5 aktif pada port ${port}`));
